...
def send_requests(params):
    ...
    if 'json_data' not in str(params_req):
        ...
        # 这里需要格式化json 可调用的
        json_data = to_json(params_req['json_data'])
...
if __name__ == '__main__':
    ...
    params = [[{'case_id': 'dome_case_001', 'url': 'https://uapi.flashparking.cn/v1/member/getMemberInfo', 'method': 'post', 'data': 'mId=$mId&accesstoken=$c_token', 'validate': "[{'eq': [{'returncode': 0}]}]", 'relevance': '[{\'plateNumber\': "[\'responsebody\'][\'list\'][0][\'parkCode\'][\'plateNumber\']"}]', 'variables_data': ['mId', 'c_token'], 'variables': "[{'login_case_001': ['mId', 'c_token']}]"}, '获取会员信息'], [{'case_id': 'dome_case_002', 'url': 'https://uapi.flashparking.cn/v1/parkOrderCard/getParkOrderCardList', 'method': 'post', 'data': 'mid=$mId&page=1&pageSize=1000', 'validate': "[{'eq': [{'returncode': 0}]}]", 'relevance': '[{\'plateNumber\': "[\'responsebody\'][\'list\'][0][\'parkCode\'][\'plateNumber\']"}]', 'variables_data': ['mId'], 'variables': "[{'login_case_001': ['mId']}]"}, '长租卡列表']]

    # print(type(aa))
    for param in params:
        bb = send_requests(param)
        print(bb)
